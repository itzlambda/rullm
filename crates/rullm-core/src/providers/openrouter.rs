...
/// OpenRouter provider implementation (wrapper around OpenAICompatibleProvider)
#[derive(Clone)]
...
impl OpenRouterProvider {
    pub fn new(config: crate::config::OpenAICompatibleConfig) -> Result<Self, LlmError> {
        let inner = OpenAICompatibleProvider::new(config, identities::OPENROUTER)?;
        Ok(Self { inner })
    }
}
...
#[async_trait::async_trait]
impl ChatCompletion for OpenRouterProvider {
